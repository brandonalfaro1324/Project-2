...
void *thread_testing(void* struct_data){
  ...
  int nblocks_each_thread = thread_node->nblocks_each_thread;
  ...
  if(currrent_thread < thread_node->threads){   
    ...
    // Begin by computing current hash value, afterwards
    // get data from left and right child and hash everything 
    ...
    // Allocated char variable "holdcharacters[]" to hold the  
    // specific parts of the main file "file_string"
